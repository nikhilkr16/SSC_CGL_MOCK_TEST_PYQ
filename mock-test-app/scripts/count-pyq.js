...
function parseFormatC(lines, filename, year, shift) {
  ...
  // First pass: find answer+explanation section
  const answerMap = {};
  let inAnswers = false;

  for (const line of lines) {
    // Detect answer/explanation section header
    if (/^(Solutions|Answers|Explanations?|Hints)/i.test(line.trim())) {
      inAnswers = true;
      continue;
    }

    if (inAnswers) {
      // Pattern: "41. ( d ) explanation text..."
  ...
  while (i < lines.length) {
    ...
    if (/^(Solutions|Answers|Explanations?|Hints)/i.test(line) && Object.keys(answerMap).length > 0) break;

    // Question: "1. question text" or "Q. 1. text"
    const qMatch = line.match(/^(\d+)\.\s+(.+)/) || line.match(/^Q[\.\s]*(\d+)[\.\s]+(.+)/i);
    if (qMatch && parseInt(qMatch[1]) <= 200) {
      const qNum = parseInt(qMatch[1]);
      let qText = qMatch[2].trim();
      i++;
      ...
      while (i < lines.length) {
        ...
        // Next question start
        if (/^\d+\.\s+/.test(cl) && parseInt(cl.match(/^(\d+)/)[1]) === qNum + 1) break;
        if (/^Q[\.\s]*\d+/i.test(cl)) break;
        if (/^(Solutions|Answers|Explanations?|Hints)/i.test(cl)) break;
...
async function main() {
  ...
  for (const file of files) {
    try {
      ...
      const lines = await extractLines(path.join(PYQ_DIR, file));
      ...
      console.log(`     Format ${format} → ${valid.length} valid (${questions.length - valid.length} skipped)`);
