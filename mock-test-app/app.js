...
function renderQuestion() {
  ...
  // Question number
  $('#question-number').textContent = `Question ${APP.currentQIndex + 1} of ${APP.flatQuestions.length}`;

  // Question text
  if (q.image_url && q.question_text.startsWith('[Image')) {
    $('#question-text').textContent = 'See the question and options in the image below.';
    $('#question-text').style.fontStyle = 'italic';
    $('#question-text').style.opacity = '0.7';
  } else {
    $('#question-text').textContent = q.question_text;
    $('#question-text').style.fontStyle = '';
    $('#question-text').style.opacity = '';
  }

  // Question image (extracted from PYQ PDFs)
  ...
  // Remove review states from options (those are only for report)
...
function renderReviewList(results) {
  ...
  for (let i = 0; i < results.questionDetails.length; i++) {
    ...
    item.innerHTML = `
      <div class="review-item-header" tabindex="0" role="button" aria-label="Expand question ${i + 1}">
      ...
      <div class="review-item-body">
        ${q.image_url ? `<div style="text-align:center;margin-bottom:var(--sp-3);"><img src="${q.image_url}" alt="Question figure" class="question-image" style="max-height:250px;" /></div>` : ''}
        <div class="review-answer-row">
          <span class="review-label">Your Answer:</span>
          <span style="color: ${q.status === 'correct' ? 'var(--color-success)' : q.status === 'wrong' ? 'var(--color-danger)' : 'var(--color-text-muted)'}">
            ${q.userAnswer ? `${q.userAnswer}. ${q.options?.[q.userAnswer] || ''}` : 'Not Attempted'}
          </span>
        </div>
        <div class="review-answer-row">
        ...
        <div class="review-answer-row" style="margin-top: var(--sp-2);">
          ...
          <span style="color: var(--color-text-muted); font-size: var(--font-xs);">${q.source_pdf || 'Unknown'} (${q.year || ''})</span>
        </div>
      </div>
    `;
    ...
    header.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); item.classList.toggle('expanded'); }
    });
